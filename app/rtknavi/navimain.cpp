    ...
    : TForm(Owner)
{
    ...
    Caption=PRGNAME;
    Caption=Caption+" ver."+VER_RTKLIB+" "+PATCH_LEVEL+" - "+ACEINNA_VER;
    DoubleBuffered=true;
	...
	int index, int freq)
{
	...
	c->Rectangle(b);
	//add start
	std::map<int,int> allsat;
	std::map<int,int>::iterator it;
    allsat.clear();
	for(i=0;i<Nsat[index]&&i<MAXSAT;i++)
	{
        if(Sat[index][i] == 0) continue;
		allsat[Sat[index][i]] = i;
	}
	int index2 = (index+1)%2;
	for(i=0;i<Nsat[index2]&&i<MAXSAT;i++)
	{
        if(Sat[index2][i] == 0) continue;
		it = allsat.find(Sat[index2][i]);
		if(it == allsat.end())
		{
			allsat[Sat[index2][i]] = -1;
        }
	}
	for(n=0,it = allsat.begin();it != allsat.end();it++)
	{
		i = it->second;
		ww=(w-16)/allsat.size();
		www=ww-2<8?ww-2:8;      // width of histogram
		x1=x0+n*(w-16)/allsat.size()+ww/2;          //x of histogram
		satno2id(it->first,id);  //get satid
		l=(q=strchr(sys,id[0]))?(int)(q-sys):5;      //index of system  "GREJCS"

		if(i != -1)
		{
			for (j=snr[0]=0;j<NFREQ;j++) {     //loop 3
				snr[j+1]=Snr[index][i][j];
				if ((freq&&freq==j+1)||((!freq||freq>NFREQ)&&snr[j+1]>snr[0])) {
					snr[0]=snr[j+1];
				}
			}
			for (j=0;j<NFREQ+2;j++) {  //loop 5
				k=j<NFREQ+1?j:0;
				y3=j<NFREQ+1?0:2;
				y2=y1-y3;
...
void __fastcall TMainForm::OpenMoniPort(int port)
{
    ...
    for (i=0;i<=MAXPORTOFF;i++) {
        ...
        if (stropen(&monistr,STR_TCPSVR,STR_MODE_RW,path)) {
            ...
            if (i>0) Caption=s.sprintf("%s ver.%s %s (%d) - %s",PRGNAME,VER_RTKLIB,PATCH_LEVEL,i+1,ACEINNA_VER);
            OpenPort=MoniPort+i;
            return;
        }
    }
...
void __fastcall TMainForm::SaveLog(void)
{
    ...
    if (SolOpt.outhead) {
        fprintf(fp,"%% program   : %s ver.%s %s - %s\n",PRGNAME,VER_RTKLIB,PATCH_LEVEL,ACEINNA_VER);
