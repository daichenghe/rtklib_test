...
static int outpos(unsigned char *buff, const char *s, const sol_t *sol, const solopt_t *opt)
{
	...
	/*p+=sprintf(p,"%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f",sep,denu[0],sep,denu[1],sep,denu[2],
		sep,norm(denu,3),sep,sol->dtr[0],sep,sol->dtr[1],sep,sol->dtr[2],sep,sol->dtr[3],sep,sol->dtr[4]);*/

	p+=sprintf(p,"%s%8.4f%s%8.4f%s%8.4f%s%8.4f",sep,denu[0],sep,denu[1],sep,denu[2],sep,norm(denu,3));

	if (cPPPAr.m_prcOptAr.iArMode>ARMODE_PPPAR_OFF)
		p+=sprintf(p,"%s%7.3f%s%6.2f%s%2d%s%2d%s%2d",sep,sol->sr*100,sep,sol->ratio,sep,sol->stat,sep,sol->nFixed[0],sep,sol->nFixed[1]);
...
extern void outResult(rtk_t *rtk,const solopt_t *sopt)
{
    ...
    else if (rtk->opt.ionoopt==IONOOPT_IF1) {
        if (PPP_Glo.outFp[OFILE_RESIC1]) outResi(PPP_Glo.outFp[OFILE_RESIC1],rtk,PPP_Glo.tNow,1,0,0);
    }
